...
from ..payloads import INTERNAL_ERROR, INVALID_PARAMS, METHOD_NOT_FOUND, error_response, normalize_prompt_result, success
...
async def handle_prompts_get(ctx: DispatchContext) -> DispatchResult:
    ...
    if prompt is None:
        payload = error_response(ctx.rpc_id, METHOD_NOT_FOUND, f"No such prompt '{prompt_name}'")
        await ctx.maybe_enqueue(payload)
        return make_result(200, json_response=True, payload=payload)
